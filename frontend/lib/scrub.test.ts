...
// A trigger price is a plain decimal like "2.5", so value-level hex scrubbing
// cannot catch it. The only safe rule is that keys which could carry order
// terms never leave the browser at all.
test("scrub drops keys that could carry order terms", () => {
  const leaky = {
    threshold: "2.5",
    thresholdE18: 2500000000000000000n.toString(),
    takeProfit: "9.75",
    direction: "below",
    minOutOrLots: "150",
    underlyingAddress: "rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe",
    terms: { anything: "at all" },
    condition: "price < 2",
    encrypted: "0xdead",
    // Legitimate analytics fields must survive.
    event: "order_sealed",
    walletConnected: true,
  };

  const out = scrub(leaky) as Record<string, unknown>;

  for (const banned of [
    "threshold", "thresholdE18", "takeProfit", "direction",
    "minOutOrLots", "underlyingAddress", "terms", "condition", "encrypted",
  ]) {
    assert.ok(!(banned in out), `key "${banned}" must never be sent`);
  }

  assert.strictEqual(out.event, "order_sealed");
  assert.strictEqual(out.walletConnected, true);
});

test("scrub drops term keys at any nesting depth", () => {
  const out = scrub({ props: { nested: { threshold: "2.5", event: "ok" } } }) as any;
  assert.ok(!("threshold" in out.props.nested), "nested threshold leaked");
  assert.strictEqual(out.props.nested.event, "ok");
});
