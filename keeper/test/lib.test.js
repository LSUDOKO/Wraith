...
import { handleFetchResultResponse, determineRelayAction, PendingBookkeeper } from "../src/lib.js";
...
test("PendingBookkeeper - add/delete flow", () => {
  const bookkeeper = new PendingBookkeeper();
  assert.strictEqual(bookkeeper.size, 0);

  // Add flow
  bookkeeper.add("inst-1", 100n);
  assert.strictEqual(bookkeeper.size, 1);
  assert.strictEqual(bookkeeper.has("inst-1"), true);
  assert.strictEqual(bookkeeper.get("inst-1"), 100n);

  bookkeeper.add("inst-2", 200n);
  assert.strictEqual(bookkeeper.size, 2);
  assert.deepStrictEqual(bookkeeper.entries(), [
    ["inst-1", 100n],
    ["inst-2", 200n],
  ]);

  // Delete flow
  const deleted1 = bookkeeper.delete("inst-1");
  assert.strictEqual(deleted1, true);
  assert.strictEqual(bookkeeper.size, 1);
  assert.strictEqual(bookkeeper.has("inst-1"), false);
  assert.strictEqual(bookkeeper.get("inst-1"), undefined);

  const deletedUnknown = bookkeeper.delete("nonexistent");
  assert.strictEqual(deletedUnknown, false);
  assert.strictEqual(bookkeeper.size, 1);
});
